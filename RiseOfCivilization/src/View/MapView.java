...
public class MapView extends JPanel {
	...
	public void paint(Graphics G) {
		...
		for(int i = 0; i < this.view.GetGameModel().GetWorkerModel().size(); i++)
		{
			...
			{
				this.view.getWorkerView().get(i).drawMove(G, this.view.GetGameModel().GetWorkerModel().get(i).getPosWhileMoving().x, this.view.GetGameModel().GetWorkerModel().get(i).getPosWhileMoving().y);
			} else {
		...
		{
			if(this.view.GetGameModel().getEnnemyModel().get(i).getVisible())
			{
